"""WebBridge channel — universal web interface for agents."""
...
class WebBridgeChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        self._server = None
        self._connected_clients: dict[str, asyncio.Queue] = {}  # api_key -> response queue
        self._ws_connections: dict[str, Any] = {}  # api_key -> websocket
    ...
    def _find_connection(self, api_key: str, client_ip: str) -> AllowedConnection | None:
        ...
        for conn in self.config.allowed_connections:
            if conn.api_key != api_key:
                continue
            # IP check: if configured, must match
    ...
    def _verify_hmac(self, data: dict) -> bool:
        ...
        if not self.config.hmac_secret:
            # No HMAC configured, skip verification
            return True
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        # Find the client by chat_id (which is the api_key)
        api_key = msg.chat_id
        ...
        # Streaming simulation - send character by character
        if msg.content:
    ...
    async def _handle_ws_client(self, websocket: Any) -> None:
        ...
        try:
            ...
            logger.info("WebBridge: Client {} disconnected", client_ip)
        except Exception as e:
    ...
    async def _handle_client_message(self, data: dict, api_key: str) -> None:
        ...
        if msg_type == "message":
            ...
            # Check if sender_id is allowed
            if not self.is_allowed(sender_id):
                logger.warning(
                    "WebBridge: Access denied for sender_id {} on channel {}",
                    sender_id, self.name
                )
                return
